...
class EnsembleAIDetector(nn.Module):
    ...
    def __init__(self, siglip_model_name: str, dinov2_model_name: str, image_size: int = 392):
        ...
        # SigLIP2 backbone. The checkpoint contains the trained weights, so the
        # architecture is built directly and populated by load_state_dict later.
...
def create_model_with_lora(
    siglip_model_name: str = "local_siglip2_so400m_patch14_384",
...
def create_siglip_transform(image_size: int = 384):
    """Create preprocessing transforms for SigLIP2."""
    return transforms.Compose([
        transforms.Resize((image_size, image_size), interpolation=transforms.InterpolationMode.BICUBIC),
        transforms.ToTensor(),
...
class AIImageDetector:
    ...
    def __init__(self, model_path: str, device: str = None):
        ...
        self.model = create_model_with_lora(
            siglip_model_name='local_siglip2_so400m_patch14_384',
        ...
        # Create processors
        self.siglip_transform = create_siglip_transform(384)
    ...
    def predict(self, image) -> dict:
        ...
        # Preprocess
        siglip_pixels = self.siglip_transform(image).unsqueeze(0).to(self.device)
